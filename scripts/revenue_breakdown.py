...
def research_one(item):
    ...
    prompt = (
        ...
        "If the filing discloses revenue at MORE THAN ONE granularity - e.g. "
        "both a coarse primary reportable segment table (used for operating "
        "income, often geographic like 'North America'/'International') AND a "
        "more granular revenue-by-product-or-service-line breakdown (often in "
        "a 'disaggregation of revenue' footnote, e.g. 'Online stores'/'AWS'/"
        "'Advertising services') - ALWAYS use the more granular product/service "
        "breakdown, not the coarse geographic one. Confirmed failure mode: "
        "Amazon returned as North America/International/AWS (3 geographic "
        "segments) instead of Online stores/Third-party seller services/AWS/"
        "Advertising services/Subscription services/Physical stores/Other (7 "
        "product/service lines) - the latter is what should be reported. "
